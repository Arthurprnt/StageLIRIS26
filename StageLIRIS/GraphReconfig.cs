...
public class GraphReconfig
{
    ...
    public GraphReconfig(Graph graph, int k, char mode, char setType)
    {
        Graph = graph;
        K = k;
        Mode = mode;
    ...
    public string ToDot()
    {
        ...
        {
            if (Reconfig.Vois[i].Count() == 0)
                dot.AppendLine($"  \"{BaseSet.toString(IndexToSet[i])}\";");
        }
        ...
        for (int i = 0; i < Reconfig.Vois.Count; i++)
        {
            ...
            {
                if (Reconfig.Vois[i][j] > i)
                    dot.AppendLine(
                        $"  \"{BaseSet.toString(IndexToSet[i])}\" -- \"{BaseSet.toString(IndexToSet[Reconfig.Vois[i][j]])}\";"
                    );
            }
        }
    ...
    public List<BaseSet> CalcAllSetsIte()
    {
        ...
        BaseSet currSet;
        if (SetType == 'I')
            currSet = new IndepSet(Graph, K);
        else if (SetType == 'D')
            currSet = new DominantSet(Graph, K);
        else
            throw new Exception("This type of set does not exist.");
                ...
                if (currSet.CanAddVert(vert))
                {
                    ...
                    if (level == K - 1)
                    {
                        if (
                            (
                                currSet is IndepSet
                                || (currSet is DominantSet currDomSet && currDomSet.IsValid())
                            )
                        )
                        {
                            ...
                            for (int prevId = 0; prevId < setId; prevId++)
                            {
                                ...
                                int intersectionSize = System.Numerics.BitOperations.PopCount(
                                    (ulong)(currSet.States & prevStates)
                                );
                                if (intersectionSize == K - 1)
                                {
                                    ...
                                    {
                                        int uniqueInCurr = BaseSet.DiffBtw(
                                            currSet.States,
                                            prevStates
                                        );
                                        int uniqueInPrev = BaseSet.DiffBtw(
                                            prevStates,
                                            currSet.States
                                        );
                                        if (Graph.Vois[uniqueInCurr].Contains(uniqueInPrev))
                                            Reconfig.AddEdge(setId, prevId);
                                    }
                                }
                            }
