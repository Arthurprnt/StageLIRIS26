...
void BuildReconfigGraph(Graph graph, int k, int[] shows)
{
    // Chronomètre
...
void BuildReconfigGraph2(Graph graph, int k, int[] shows)
{
    // Chronomètre
    Stopwatch stopwatch = Stopwatch.StartNew();
...
void GetDiamOfIsGraph(string filePath, int k, int[] shows, int version)
{
    ...
    {
        case 1:
            BuildReconfigGraph(graph, k, shows);
            break;
        case 2:
            BuildReconfigGraph2(graph, k, shows);
            break;
    }
}

GetDiamOfIsGraph("graphs/hog/graph_1173.txt", 3, [1, 0, 0, 0], 1);

Console.WriteLine();
Console.WriteLine();

GetDiamOfIsGraph("graphs/hog/graph_1173.txt", 3, [1, 0, 0, 0], 2);